...
```bash
cloudlib$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
cloudlib$ sam local start-api
cloudlib$ curl http://localhost:3000/
```
...
```bash
cloudlib$ sam logs -n NetCodeWebAPIServerless --stack-name cloudlib --tail
```
...
```bash
cloudlib$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name cloudlib
```
